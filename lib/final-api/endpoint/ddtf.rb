      ...
      class << self
        def registered(app)
          ...
          app.post '/ddtf/tests/:id/retest' do
            ...
            puts retest_data
            stripped_runtime_config = retest_data['runtimeConfigFields'].reject { |key, _| key[:definition].downcase.include?("webserver") }
            retest_data['runtimeConfigFields'] = stripped_runtime_config
            retest_data.to_json
          end
